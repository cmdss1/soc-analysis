...
| **Chrome still warns on HTTPS (“Issued by mitmproxy”)** | Linux Chrome trusts **`~/.pki/nssdb`**, not only the system CA bundle. The mitm CA must be imported **after** **`kasm_default_profile.sh`** copies **`/home/kasm-default-profile`** into **`/home/kasm-user`** — otherwise the copied profile **overwrites** `.pki/nssdb` and the earlier install is lost. Current image uses **`kasm-mitm-nss-trust.sh`** in the startup chain immediately after the profile script. |
